const { initializeApp, getApps, cert } = require('firebase-admin/app');
const { getAuth } = require('firebase-admin/auth');

// Firebase Admin inisializasiyası (Açarlar birbaşa koda daxil edilib)
if (getApps().length === 0) {
  initializeApp({
    credential: cert({
      projectId: "rahatizle-yeni",[cite: 1]
      clientEmail: "firebase-adminsdk-fbsvc@rahatizle-yeni.iam.gserviceaccount.com",[cite: 1]
      privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"[cite: 1]
    }),
  });
}

// Vercel Serverless Function (CommonJS formatı)
module.exports = async (req, res) => {
  // Yalnız POST sorğularına icazə veririk
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Yalnız POST sorğuları qəbul edilir' });
  }

  const { email, newPassword } = req.body;

  if (!email || !newPassword) {
    return res.status(400).json({ error: 'E-poçt və ya yeni şifrə daxil edilməyib' });
  }

  try {
    // E-poçta əsasən istifadəçini tapırıq
    const userRecord = await getAuth().getUserByEmail(email);
    
    // Şifrəni kənardan qapalı şəkildə yeniləyirik
    await getAuth().updateUser(userRecord.uid, {
      password: newPassword,
    });
    
    res.status(200).json({ message: 'Şifrə uğurla yeniləndi!' });
  } catch (error) {
    console.error("Backend Firebase Xətası:", error);
    res.status(500).json({ error: error.message });
  }
};